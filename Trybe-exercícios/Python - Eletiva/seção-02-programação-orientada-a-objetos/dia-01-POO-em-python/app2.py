...
class Questao:
    def __init__(self, enunciado: str, resposta_correta: int | str) -> None:
        self.enunciado = enunciado
        self.resposta_correta = resposta_correta

    def resolver(self, resposta: int | str) -> bool:
...
def main() -> None:
    ...
    Sempre que uma classe utiliza outra classe como parâmetro ela está
    aplicando a herança. Tornando-se classe filha da mesma.

    Como o super() ajudou a reaproveitar código?
    Poupou a repetição de código utilizando os métodos já implementados
    da classe Mãe.

    Qual o motivo do self existir? Seria possível alterar o objeto sem ele?
    O self é o objeto em si, sem ele não seria possível alterar o objeto.

    Rode mypy app.py. O mypy não reclama. Por quê?
    Pois ele não verifica faltas de tipagem explícitas por padrão.

    Rode mypy --strict app.py. O mypy passa a reclamar. Por quê?
    Pois com a flag strict ele identifica
    e aponta faltas de tipagem explícitas.
    """
